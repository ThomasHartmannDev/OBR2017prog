...
//garra
void motor_baixo(byte forca)
{
  digitalWrite(A2_menos,HIGH);
  analogWrite(A2_mais_pwm,forca);
   digitalWrite(B2_menos,HIGH);
  analogWrite(B2_mais_pwm,forca);
  }
  //função para voltar()
void motor_cima(byte forca)
{
   digitalWrite(A2_menos,LOW);
   analogWrite(A2_mais_pwm,forca); 
   digitalWrite(B2_menos,LOW);
   analogWrite(B2_mais_pwm,forca);       
}
//função para parar()
void motor_desliga()
{
   digitalWrite(A2_menos,LOW);
   analogWrite(A2_mais_pwm,0);
   digitalWrite(B2_menos,LOW);
   analogWrite(B2_mais_pwm,0);
}
